...
GinlongSrv.prototype._calculateStats = function() {
  ...
  this._db.get(statSql, (err, row) => {
    ...
    const recvStmt = 'INSERT INTO received (port, time, data, json) values (9999, ?, ?, ?)';
    if (row.timestamp < row.time_sod) {
      ...
      this._db.run(logsStmt, [row.time_sod - 60, row.serial, row.e_cur, row.e_today, row.e_total, row.e_prev], (err) => {
        if (err) {
          console.error('calcStats: update before error: ' + err.message);
        }
      });
      this._db.run(recvStmt, [row.time_sod - 60, row, JSON.stringify(row)], (err) => {
        if (err) {
          console.error('calcStats recv: insert before error: ' + err.message);
        }
      });
      // after midnight, zero out e_today
      this._db.run(logsStmt, [row.time_sod + 60, row.serial, 0, 0, row.e_total, row.e_prev], (err) => {
        if (err) {
      ...
      console.warn(getTimestamp() + ' - adding log every hour without data');
      // add a logline for every hour without logging 
      this._db.run(logsStmt, [row.time_now , row.serial, row.e_cur, row.e_today, row.e_total, row.e_prev], (err) => {
        if (err) {
          console.error('calcStats: insert error: ' + err.message);
        }
      });
      this._db.run(recvStmt, [row.time_now, row, JSON.stringify(row)], (err) => {
        if (err) {
