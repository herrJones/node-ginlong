...
GinlongSrv.prototype._calculateStats = function() {
  ...
  this._db.get(statSql, (err, row) => {
    ...
    if (row.timestamp < row.time_sod) {
      ...
      this._db.run(logsStmt, [row.time_now - 15, row.serial, row.e_cur, row.e_today, row.e_total, row.e_prev], (err) => {
        if (err) {
          console.error('calcStats: insert error: ' + err.message);
        }
      });
      this._db.run(recvStmt, [row.time_now -15, row, JSON.stringify(row)], (err) => {
        if (err) {
