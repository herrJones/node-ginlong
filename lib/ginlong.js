...
GinlongSrv.prototype._processData = function(element) {
  ...
  if (protoData != null) {
    let invID = this._storeInverter(protoData);
...
GinlongSrv.prototype._calcDateDiff = function(diff) {
  // calculate 1 day in unix time format
  let time = (1000 * 60 * 60 * 24) * 1000;
...
GinlongSrv.prototype._cleanupData = function() {
  setTimeout(this._cleanupData, 1000*60*30);

  // cleanup raw data
  let diffTime = this._calcDateDiff(this.settings.purgeRaw.after);
  let lokiData = this._db.getCollection('raw');
  lokiData.findAndRemove({ 'time' : { '$lt' : diffTime}});

  // cleanup log data
  diffTime = this._calcDateDiff(this.settings.purgeLog.after);
...
GinlongSrv.prototype.getStats = function() {
  ...
  inverters.data.forEach(inverter => {
    ...
    result = {
      ...
      'time'    : Math.round(curDate.getTime() / 1000)
    };
    ...
    if ((logDate.getDate() === curDate.getDate()) &&
      ...
      newLog = {
        ...
        e_prev: 0 ,p_status: curLog[0].p_status
      }
      logs.insertOne(newLog);
